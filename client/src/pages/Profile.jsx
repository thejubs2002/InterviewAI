...
import { User, Briefcase, Target, Code, Save } from 'lucide-react';
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-[var(--color-text-secondary)]">Manage your personal information and interview preferences.</p>
      </motion.div>
      ...
      <div className="card space-y-5">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input name="title" value={form.title} onChange={handleChange} placeholder="e.g., Full Stack Developer" className="input-field" />
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium mb-2">Bio</label>
          <textarea name="bio" value={form.bio} onChange={handleChange} rows={3} placeholder="Tell us about yourself..." className="input-field resize-y" maxLength={500} />
          <p className="text-xs text-[var(--color-text-tertiary)] mt-1">{form.bio.length}/500</p>
        </div>
      ...
      <div className="card space-y-5">
        ...
        <div>
          ...
          <input name="skills" value={form.skills} onChange={handleChange} placeholder="JavaScript, React, Node.js, Python" className="input-field" />
        </div>

        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium mb-2">Target Role</label>
            <input name="targetRole" value={form.targetRole} onChange={handleChange} placeholder="e.g., Senior SWE" className="input-field" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-2">Target Company</label>
            <input name="targetCompany" value={form.targetCompany} onChange={handleChange} placeholder="e.g., Google" className="input-field" />
          </div>
        </div>
      </div>
